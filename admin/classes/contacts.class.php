<?php
	...
	$countalldata=Admin::countAllData('tbl_shisobright');
	$allContacts = Admin::getAllData($page,'tbl_shisobright');
	
	if(isset($_GET['delete'])){ 
		Admin::singleDelete($_GET['delete'],'tbl_shisobright');
	...
	if(isset($_POST['delall'])){ 
		if(!isset($_POST["chkarray"])){
			...
			for($a=0; $a < count($_POST["chkarray"]); $a++){
				Admin::singleDelete($_POST["chkarray"][$a],'tbl_shisobright');
			}
			echo '<script>window.location="home.php"</script>';
		}
	}
	if(isset($_POST['expBtn'])){ 
		$expdata = Admin::getAllDataExp('tbl_shisobright');
